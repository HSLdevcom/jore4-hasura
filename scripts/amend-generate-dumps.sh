...
# Make sure the generic schema is selected, otherwise dumps are not generated correctly. Should be by default.
SELECTED_MIGRATION_DIRECTORY=$(grep "migrations_directory: " ../config.yaml | cut -d ":" -f2 | tr -d " ")
if [ "$SELECTED_MIGRATION_DIRECTORY" != "migrations/generic" ]; then
  echo "Generic schema not selected. Aborting."
  exit 1
fi
...
# It is important that DB is all ready and migrations have been ran, before we try to generate the dumps.
