...
1. Run `hasura migrate ...` to read or write DB migration files. Note that this runs the migrations from your local workspace, not the docker image. Should make sure that's Hasura's `config.yaml` points to the correct migrations folder that is intended to be used.
...
Q: How to add check contraints to tables?
...
Note: if you modify the metadata within the docker container (http://localhost:3201), the yamls files are not refreshed. Do make sure you make the changes using `hasura console`.
