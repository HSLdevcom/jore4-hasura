...
import { infrastructureLinks as sourceInfrastructureLinks } from '@datasets/routesAndJourneyPatterns/infrastructure-links';
...
describe('Function service_pattern.get_distances_between_stop_points', () => {
  ...
  const getDistancesBetweenStopPoints = async (
    ...
    const response = await db.singleQuery(
      dbConnectionPool,
      `SELECT * FROM service_pattern.get_distances_between_stop_points('{${journeyPatternIds}}'::uuid[], '${observationDate.toISOString()}'::date)`,
    );
