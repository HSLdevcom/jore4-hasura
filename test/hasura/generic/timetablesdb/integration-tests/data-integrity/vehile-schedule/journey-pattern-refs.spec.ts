...
describe('Denormalized references to journey patterns in vehicle services', () => {
  ...
  const buildPartialInsertVehicleJourneyMutation = (
  ...
  const buildPartialUpdateVehicleJourneyMutation = (
  ...
  const buildPartialDeletePassingTimeMutation = (vehicleJourneyId: UUID) => `
    timetables {
  ...
  const buildPartialDeleteVehicleJourneyMutation = (vehicleJourneyId: UUID) => `
    timetables {
  ...
  describe('on vehicle journey insert', () => {
    it('should increment count on existing row when one exists', async () => {
      ...
      const insertMutation = `mutation {
        ${buildPartialInsertVehicleJourneyMutation(newVehicleJourney)}
      }`;
  ...
  describe('on journey pattern ref delete', () => {
    it('should correctly decrement count', async () => {
      const deleteMutation = `
        mutation {
          ${buildPartialDeletePassingTimeMutation(
            vehicleJourneysByName.v1MonFriJourney1.vehicle_journey_id,
          )}
          ${buildPartialDeleteVehicleJourneyMutation(
            vehicleJourneysByName.v1MonFriJourney1.vehicle_journey_id,
          )}
        }`;
    ...
    it('should delete the row when count reaches zero', async () => {
      const deleteMutation = `
        mutation {
          outbound_journey1_tpt_delete: ${buildPartialDeletePassingTimeMutation(
            vehicleJourneysByName.v1MonFriJourney1.vehicle_journey_id,
          )}
          outbound_journey1_vj_delete: ${buildPartialDeleteVehicleJourneyMutation(
            vehicleJourneysByName.v1MonFriJourney1.vehicle_journey_id,
          )}

          outbound_journey2_tpt_delete: ${buildPartialDeletePassingTimeMutation(
            vehicleJourneysByName.v1MonFriJourney3.vehicle_journey_id,
          )}
          outbound_journey2_vj_delete: ${buildPartialDeleteVehicleJourneyMutation(
            vehicleJourneysByName.v1MonFriJourney3.vehicle_journey_id,
          )}
        }`;
      const deleteRes = await postQuery(deleteMutation);
  ...
  describe('on vehicle journey update', () => {
    it('should update counts on affected rows when foreign keys were changed', async () => {
      const updateMutation = `mutation {
        ${buildPartialUpdateVehicleJourneyMutation(
          vehicleJourneysByName.v1MonFriJourney1.vehicle_journey_id,
          {
            block_id: vehicleServiceBlocksByName.v1Sat.block_id,
          },
      ...
      expect(response.rows).toEqual(
        expect.arrayContaining([
          ...
          // which is in different veihcle service -> new row.
          {
            vehicle_service_id: vehicleServicesByName.v1Sat.vehicle_service_id,
            journey_pattern_id:
