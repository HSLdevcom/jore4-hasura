...
describe("Insert scheduled stop point into journey pattern", () => {
  ...
  describe("when stop's link is traversed in a direction not compatible with the stop's direction", () => {
    const toBeInserted = createToBeInserted(
      journeyPatterns[0].journey_pattern_id,
    ...
    shouldReturnErrorResponse(
      toBeInserted,
      "found stop in journey pattern, whose infra link is traversed in a direction not compatible with the stop's directions"
    );
