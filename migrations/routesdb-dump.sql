...
WITH RECURSIVE
  ...
  entity AS (
    SELECT r.route_id AS id, r.validity_start, r.validity_end, r.label AS key1, r.direction AS key2, r.variant as key3, r.priority
    FROM route.route r
...
CREATE TABLE route.route (
    ...
    CONSTRAINT route_variant_check CHECK ((variant >= 0))
);
