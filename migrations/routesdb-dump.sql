...
--
-- Name: FUNCTION get_distances_between_stop_points_for_journey_patterns(route_ids uuid[], route_priority integer, observation_date date); Type: COMMENT; Schema: service_pattern; Owner: dbhasura
--

COMMENT ON FUNCTION service_pattern.get_distances_between_stop_points_for_journey_patterns(route_ids uuid[], route_priority integer, observation_date date) IS 'Get the distances between scheduled stop points (in metres) for the journey/service patterns resolved from the route identifiers.';
...
CREATE FUNCTION service_pattern.get_distances_between_stop_points(journey_pattern_id uuid, observation_date date, include_draft_stops boolean) RETURNS TABLE(journey_pattern_id uuid, route_id uuid, stop_interval_sequence integer, start_stop_label text, end_stop_label text, distance_in_metres double precision)
    LANGUAGE sql STABLE PARALLEL SAFE
    AS $$
...
CREATE FUNCTION service_pattern.get_distances_between_stop_points(journey_pattern_ids uuid[], observation_date date, include_draft_stops boolean) RETURNS TABLE(journey_pattern_id uuid, route_id uuid, stop_interval_sequence integer, start_stop_label text, end_stop_label text, distance_in_metres double precision)
    LANGUAGE sql STABLE PARALLEL SAFE
    AS $$
WITH RECURSIVE
...
CREATE FUNCTION service_pattern.get_distances_between_stop_points_for_journey_patterns(route_ids uuid[], route_priority integer, observation_date date) RETURNS SETOF service_pattern.distance_between_stops_calculation
    LANGUAGE sql STABLE PARALLEL SAFE
    AS $$
SELECT
  stop_interval_length.journey_pattern_id,
  route_info.route_id,
  route_info.route_priority,
  observation_date,
  stop_interval_length.stop_interval_sequence,
  stop_interval_length.start_stop_label,
  stop_interval_length.end_stop_label,
  stop_interval_length.distance_in_metres
FROM (
  SELECT
    r.route_id,
  ...
  WHERE
    ...
    AND r.priority = route_priority
    AND internal_utils.daterange_closed_upper(r.validity_start, r.validity_end) @> observation_date
  GROUP BY r.route_id
) route_info
JOIN LATERAL (
  SELECT *
  FROM service_pattern.get_distances_between_stop_points(journey_pattern_ids, observation_date, false)
) stop_interval_length USING (route_id)
ORDER BY journey_pattern_id ASC, stop_interval_sequence ASC
$$;


ALTER FUNCTION service_pattern.get_distances_between_stop_points_for_journey_patterns(route_ids uuid[], route_priority integer, observation_date date) OWNER TO dbhasura;
