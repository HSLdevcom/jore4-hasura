...
--
-- Name: FUNCTION check_infra_link_stop_refs_with_new_scheduled_stop_point(replace_scheduled_stop_point_id uuid, new_located_on_infrastructure_link_id uuid, new_measured_location public.geography, new_direction text, new_label text, new_validity_start date, new_validity_end date, new_priority integer, new_vehicle_mode text); Type: COMMENT; Schema: journey_pattern; Owner: dbhasura
--

COMMENT ON FUNCTION journey_pattern.check_infra_link_stop_refs_with_new_scheduled_stop_point(replace_scheduled_stop_point_id uuid, new_located_on_infrastructure_link_id uuid, new_measured_location public.geography, new_direction text, new_label text, new_validity_start date, new_validity_end date, new_priority integer, new_vehicle_mode text) IS 'Check whether the journey pattern''s / route''s links and stop points still correspond to each other
...
--
-- Name: FUNCTION maximum_priority_validity_spans(entity_type text, filter_route_labels text[], filter_validity_start date, filter_validity_end date, upper_priority_limit integer, replace_scheduled_stop_point_id uuid, new_scheduled_stop_point_id uuid, new_located_on_infrastructure_link_id uuid, new_measured_location public.geography, new_direction text, new_label text, new_validity_start date, new_validity_end date, new_priority integer, new_vehicle_mode text); Type: COMMENT; Schema: journey_pattern; Owner: dbhasura
--

COMMENT ON FUNCTION journey_pattern.maximum_priority_validity_spans(entity_type text, filter_route_labels text[], filter_validity_start date, filter_validity_end date, upper_priority_limit integer, replace_scheduled_stop_point_id uuid, new_scheduled_stop_point_id uuid, new_located_on_infrastructure_link_id uuid, new_measured_location public.geography, new_direction text, new_label text, new_validity_start date, new_validity_end date, new_priority integer, new_vehicle_mode text) IS 'Find the validity time spans of highest priority in the given time span for entities of the given type (routes or
    ...
    The new_vehicle_mode parameter defines the vehicle mode of the new hypothetical scheduled stop point. Vehicle
    mode affects route validity checks: a scheduled stop point is only considered to belong to a route if its
    vehicle_mode matches the route line''s primary_vehicle_mode.';
...
--
-- Name: FUNCTION new_scheduled_stop_point_if_id_given(new_scheduled_stop_point_id uuid, new_located_on_infrastructure_link_id uuid, new_measured_location public.geography, new_direction text, new_label text, new_validity_start date, new_validity_end date, new_priority integer, new_vehicle_mode text); Type: COMMENT; Schema: service_pattern; Owner: dbhasura
--

COMMENT ON FUNCTION service_pattern.new_scheduled_stop_point_if_id_given(new_scheduled_stop_point_id uuid, new_located_on_infrastructure_link_id uuid, new_measured_location public.geography, new_direction text, new_label text, new_validity_start date, new_validity_end date, new_priority integer, new_vehicle_mode text) IS 'Conditionally returns a row representing a new scheduled_stop_point, or nothing.
...
COMMENT ON VIEW service_pattern.scheduled_stop_points_with_infra_link_data IS 'Contains scheduled_stop_points enriched with some infra link data and vehicle_mode.';
...
CREATE FUNCTION infrastructure_network.resolve_point_to_closest_link(geog public.geography, filter_vehicle_submode text DEFAULT 'generic_bus'::text) RETURNS SETOF infrastructure_network.infrastructure_link
    LANGUAGE sql STABLE STRICT PARALLEL SAFE
    AS $$
SELECT link.*
FROM (
...
--
-- Name: check_infra_link_stop_refs_with_new_scheduled_stop_point(uuid, uuid, public.geography, text, text, date, date, integer, text); Type: FUNCTION; Schema: journey_pattern; Owner: dbhasura
--

CREATE FUNCTION journey_pattern.check_infra_link_stop_refs_with_new_scheduled_stop_point(replace_scheduled_stop_point_id uuid, new_located_on_infrastructure_link_id uuid, new_measured_location public.geography, new_direction text, new_label text, new_validity_start date, new_validity_end date, new_priority integer, new_vehicle_mode text DEFAULT NULL::text) RETURNS SETOF journey_pattern.journey_pattern
...
--
-- Name: get_broken_route_journey_patterns(uuid[], uuid, uuid, public.geography, text, text, date, date, integer, text); Type: FUNCTION; Schema: journey_pattern; Owner: dbhasura
--

CREATE FUNCTION journey_pattern.get_broken_route_journey_patterns(filter_route_ids uuid[], replace_scheduled_stop_point_id uuid DEFAULT NULL::uuid, new_located_on_infrastructure_link_id uuid DEFAULT NULL::uuid, new_measured_location public.geography DEFAULT NULL::public.geography, new_direction text DEFAULT NULL::text, new_label text DEFAULT NULL::text, new_validity_start date DEFAULT NULL::date, new_validity_end date DEFAULT NULL::date, new_priority integer DEFAULT NULL::integer, new_vehicle_mode text DEFAULT NULL::text) RETURNS SETOF journey_pattern.journey_pattern
    LANGUAGE sql STABLE PARALLEL SAFE
    AS $$
WITH RECURSIVE
  new_ssp_param AS (
  ...
  prioritized_route AS (
    ...
    FROM route.route r
           JOIN route.line l ON l.line_id = r.on_line_id
           ...
           JOIN prioritized_route r
                ON r.route_id = sspijp.route_id
                  ...
                  AND internal_utils.daterange_closed_upper(ssp.priority_span_validity_start, ssp.priority_span_validity_end) &&
                      internal_utils.daterange_closed_upper(r.validity_start, r.validity_end)
                  -- filter out stop point instances whose vehicle mode does not match the route's line vehicle mode
                  AND (ssp.vehicle_mode IS NULL OR r.primary_vehicle_mode IS NULL
                       OR ssp.vehicle_mode = r.primary_vehicle_mode)
                  ...
                  (ssp.scheduled_stop_point_id IS NULL
                    AND NOT EXISTS(
                      ...
                      FROM route.route full_route
                             JOIN route.line ghost_line ON ghost_line.line_id = full_route.on_line_id
                      ...
                      WHERE full_route.route_id = jp.on_route_id
                        ...
                        AND (any_ssp.vehicle_mode IS NULL OR ghost_line.primary_vehicle_mode IS NULL
                             OR any_ssp.vehicle_mode = ghost_line.primary_vehicle_mode)
                      )
                    )                                     AS is_ghost_ssp
...
--
-- Name: maximum_priority_validity_spans(text, text[], date, date, integer, uuid, uuid, uuid, public.geography, text, text, date, date, integer, text); Type: FUNCTION; Schema: journey_pattern; Owner: dbhasura
--

CREATE FUNCTION journey_pattern.maximum_priority_validity_spans(entity_type text, filter_route_labels text[], filter_validity_start date DEFAULT NULL::date, filter_validity_end date DEFAULT NULL::date, upper_priority_limit integer DEFAULT NULL::integer, replace_scheduled_stop_point_id uuid DEFAULT NULL::uuid, new_scheduled_stop_point_id uuid DEFAULT NULL::uuid, new_located_on_infrastructure_link_id uuid DEFAULT NULL::uuid, new_measured_location public.geography DEFAULT NULL::public.geography, new_direction text DEFAULT NULL::text, new_label text DEFAULT NULL::text, new_validity_start date DEFAULT NULL::date, new_validity_end date DEFAULT NULL::date, new_priority integer DEFAULT NULL::integer, new_vehicle_mode text DEFAULT NULL::text) RETURNS TABLE(id uuid, validity_start date, validity_end date)
    LANGUAGE sql STABLE PARALLEL SAFE
    AS $$
WITH RECURSIVE
  ...
  entity AS (
    SELECT r.route_id AS id, r.validity_start, r.validity_end, r.label AS key1, r.direction AS key2, NULL AS key3, r.priority
    FROM route.route r
...
--
-- Name: new_scheduled_stop_point_if_id_given(uuid, uuid, public.geography, text, text, date, date, integer, text); Type: FUNCTION; Schema: service_pattern; Owner: dbhasura
--

CREATE FUNCTION service_pattern.new_scheduled_stop_point_if_id_given(new_scheduled_stop_point_id uuid DEFAULT NULL::uuid, new_located_on_infrastructure_link_id uuid DEFAULT NULL::uuid, new_measured_location public.geography DEFAULT NULL::public.geography, new_direction text DEFAULT NULL::text, new_label text DEFAULT NULL::text, new_validity_start date DEFAULT NULL::date, new_validity_end date DEFAULT NULL::date, new_priority integer DEFAULT NULL::integer, new_vehicle_mode text DEFAULT NULL::text) RETURNS TABLE(scheduled_stop_point_id uuid, measured_location public.geography, located_on_infrastructure_link_id uuid, direction text, label text, validity_start date, validity_end date, priority integer, relative_distance_from_infrastructure_link_start double precision, vehicle_mode text)
    LANGUAGE sql STABLE
    AS $$
