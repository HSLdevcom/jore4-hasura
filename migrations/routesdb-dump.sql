...
CREATE FUNCTION service_pattern.get_distances_between_stop_points(journey_pattern_id uuid, observation_date date) RETURNS TABLE(journey_pattern_id uuid, route_id uuid, stop_interval_sequence integer, start_stop_label text, end_stop_label text, distance_in_metres double precision)
    LANGUAGE sql STABLE PARALLEL SAFE
    AS $$
...
CREATE FUNCTION service_pattern.get_distances_between_stop_points(journey_pattern_ids uuid[], observation_date date) RETURNS TABLE(journey_pattern_id uuid, route_id uuid, stop_interval_sequence integer, start_stop_label text, end_stop_label text, distance_in_metres double precision)
    LANGUAGE sql STABLE PARALLEL SAFE
    AS $$
WITH RECURSIVE
...
JOIN LATERAL (
  SELECT *
  FROM service_pattern.get_distances_between_stop_points(journey_pattern_ids, observation_date)
