...
--
-- Name: FUNCTION find_route_ids(line_id uuid, route_priority integer, observation_date date); Type: COMMENT; Schema: route; Owner: dbhasura
--

COMMENT ON FUNCTION route.find_route_ids(line_id uuid, route_priority integer, observation_date date) IS 'Find the IDs for routes that belong to the given line, have the given priority and are valid on the given date.';
...
--
-- Name: FUNCTION find_scheduled_stop_point_locations_in_journey_pattern(filter_journey_pattern_id uuid, observation_date date); Type: COMMENT; Schema: service_pattern; Owner: dbhasura
--

COMMENT ON FUNCTION service_pattern.find_scheduled_stop_point_locations_in_journey_pattern(filter_journey_pattern_id uuid, observation_date date) IS 'Get location information for scheduled stop points in journey/service pattern.';

--
-- Name: FUNCTION get_distances_between_stop_points(journey_pattern_id uuid, observation_date date); Type: COMMENT; Schema: service_pattern; Owner: dbhasura
--

COMMENT ON FUNCTION service_pattern.get_distances_between_stop_points(journey_pattern_id uuid, observation_date date) IS 'Get the distances between scheduled stop points (in metres) for the given journey/service pattern.';

--
-- Name: FUNCTION get_distances_between_stop_points(journey_pattern_ids uuid[], observation_date date); Type: COMMENT; Schema: service_pattern; Owner: dbhasura
--

COMMENT ON FUNCTION service_pattern.get_distances_between_stop_points(journey_pattern_ids uuid[], observation_date date) IS 'Get the distances between scheduled stop points (in metres) for the given journey/service patterns.';

--
-- Name: FUNCTION get_distances_between_stop_points_for_journey_patterns(line_id uuid, route_priority integer, observation_date date); Type: COMMENT; Schema: service_pattern; Owner: dbhasura
--

COMMENT ON FUNCTION service_pattern.get_distances_between_stop_points_for_journey_patterns(line_id uuid, route_priority integer, observation_date date) IS 'Get the distances between scheduled stop points (in metres) for the journey/service patterns resolved from the routes belonging to the given line.';
...
--
-- Name: find_route_ids(uuid, integer, date); Type: FUNCTION; Schema: route; Owner: dbhasura
--

CREATE FUNCTION route.find_route_ids(line_id uuid, route_priority integer, observation_date date) RETURNS SETOF uuid
    LANGUAGE sql STABLE PARALLEL SAFE
    AS $$
SELECT route_id
FROM route.route r
...
ALTER FUNCTION route.find_route_ids(line_id uuid, route_priority integer, observation_date date) OWNER TO dbhasura;
...
--
-- Name: find_scheduled_stop_point_locations_in_journey_pattern(uuid, date); Type: FUNCTION; Schema: service_pattern; Owner: dbhasura
--

CREATE FUNCTION service_pattern.find_scheduled_stop_point_locations_in_journey_pattern(filter_journey_pattern_id uuid, observation_date date) RETURNS TABLE(journey_pattern_id uuid, scheduled_stop_point_sequence integer, scheduled_stop_point_id uuid, label text, measured_location public.geography, located_on_infrastructure_link_id uuid, direction text, timing_place_id uuid)
    LANGUAGE sql STABLE PARALLEL SAFE
    AS $$
...
ALTER FUNCTION service_pattern.find_scheduled_stop_point_locations_in_journey_pattern(filter_journey_pattern_id uuid, observation_date date) OWNER TO dbhasura;

--
-- Name: get_distances_between_stop_points(uuid, date); Type: FUNCTION; Schema: service_pattern; Owner: dbhasura
--

CREATE FUNCTION service_pattern.get_distances_between_stop_points(journey_pattern_id uuid, observation_date date) RETURNS TABLE(route_id uuid, journey_pattern_id uuid, stop_interval_sequence integer, start_stop_label text, end_stop_label text, distance_in_metres double precision)
    LANGUAGE sql STABLE PARALLEL SAFE
    AS $$
...
ALTER FUNCTION service_pattern.get_distances_between_stop_points(journey_pattern_id uuid, observation_date date) OWNER TO dbhasura;

--
-- Name: get_distances_between_stop_points(uuid[], date); Type: FUNCTION; Schema: service_pattern; Owner: dbhasura
--

CREATE FUNCTION service_pattern.get_distances_between_stop_points(journey_pattern_ids uuid[], observation_date date) RETURNS TABLE(route_id uuid, journey_pattern_id uuid, stop_interval_sequence integer, start_stop_label text, end_stop_label text, distance_in_metres double precision)
    LANGUAGE sql STABLE PARALLEL SAFE
    AS $$
WITH RECURSIVE
...
route_link_traversal AS (
  SELECT
    ...
    ilar.infrastructure_link_sequence,
    CASE
      WHEN line_endpoints IS NULL THEN
        CASE
          WHEN l.estimated_length_in_metres IS NOT NULL THEN l.estimated_length_in_metres
            -- CRS transformation should result in a metric coordinate system.
            -- E.g. EPSG:4326 is not like one.
          ELSE ST_3DLength(ST_Transform(l.shape::geometry, internal_utils.determine_srid(l.shape)))
        END
      ELSE
        ST_3DLength( -- take elevation changes into account, hence 3D length
          ST_LineSubstring(
            -- CRS transformation should result in a metric coordinate system.
            -- E.g. EPSG:4326 is not like one.
...
ALTER FUNCTION service_pattern.get_distances_between_stop_points(journey_pattern_ids uuid[], observation_date date) OWNER TO dbhasura;

--
-- Name: get_distances_between_stop_points_for_journey_patterns(uuid, integer, date); Type: FUNCTION; Schema: service_pattern; Owner: dbhasura
--

CREATE FUNCTION service_pattern.get_distances_between_stop_points_for_journey_patterns(line_id uuid, route_priority integer, observation_date date) RETURNS TABLE(route_id uuid, journey_pattern_id uuid, stop_interval_sequence integer, start_stop_label text, end_stop_label text, distance_in_metres double precision)
    LANGUAGE sql STABLE PARALLEL SAFE
    AS $$
SELECT *
...
ALTER FUNCTION service_pattern.get_distances_between_stop_points_for_journey_patterns(line_id uuid, route_priority integer, observation_date date) OWNER TO dbhasura;
