...
COMMENT ON TABLE timing_pattern.timing_point IS 'A POINT against which the timing information necessary to build schedules may be recorded. In HSL context this is "Hastus paikka"; also at HSL every TIMING POINT must also be a SCHEDULED STOP POINT. Transmodel: https://www.transmodel-cen.eu/model/index.htm?goto=2:3:2:709 ';

-------------------- Service Pattern --------------------

ALTER TABLE internal_service_pattern.scheduled_stop_point_invariant
  ADD COLUMN timing_point_id TEXT REFERENCES timing_pattern.timing_point (timing_point_id) NULL;
COMMENT ON COLUMN internal_service_pattern.scheduled_stop_point_invariant.timing_tag_label IS 'Optional reference to a TIMING POINT. If NULL, the scheduled stop point is not used for timing.';
