...
-- note: removed the deleting part from the function, only doing the side-effects
...
-- note: removed the inserting part from the function, only doing the side-effects
...
-- make sure the invariant label is created (if necessary) when the scheduled stop point is inserted
-- note: switched from the view's INSTEAD OF TRIGGER to BEFORE INSERT TRIGGER
...
-- note: removed the updating part from the function, only doing the side-effects
...
-- make sure the invariant label is updated when the scheduled stop point is updated
-- note: switched from the view's INSTEAD OF TRIGGER to BEFORE UPDATE TRIGGER
...
-- make sure the invariant label is removed (if necessary) when the scheduled stop point is deleted
-- note: switched from the view's INSTEAD OF TRIGGER to AFTER DELETE TRIGGER
...
-- also note that:
-- - relative_distance_from_infrastructure_link_start is now calculated instead of retrieved through the view
-- - closest_point_on_infrastructure_link is now calculated instead of retrieved through the view
DROP FUNCTION service_pattern.get_scheduled_stop_points_with_new(replace_scheduled_stop_point_id uuid, new_scheduled_stop_point_id uuid, new_located_on_infrastructure_link_id uuid, new_measured_location public.geography, new_direction text, new_label text, new_validity_start date, new_validity_end date, new_priority integer);
CREATE FUNCTION service_pattern.get_scheduled_stop_points_with_new(replace_scheduled_stop_point_id uuid DEFAULT NULL::uuid, new_scheduled_stop_point_id uuid DEFAULT NULL::uuid, new_located_on_infrastructure_link_id uuid DEFAULT NULL::uuid, new_measured_location public.geography DEFAULT NULL::public.geography, new_direction text DEFAULT NULL::text, new_label text DEFAULT NULL::text, new_validity_start date DEFAULT NULL::date, new_validity_end date DEFAULT NULL::date, new_priority integer DEFAULT NULL::integer)
    RETURNS TABLE (
