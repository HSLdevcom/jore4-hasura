...
COMMENT ON TRIGGER queue_verify_infra_link_stop_refs_on_ilar_update_trigger
  ON route.infrastructure_link_along_route
  IS 'Trigger to verify the infra link <-> scheduled stop point references.
      Updating an infrastructure_link_along_route -row can break the route consistency such that a stop point
      might be left on an infra link which is not anymore part of its journey pattern''s route.';
...
COMMENT ON TRIGGER queue_verify_infra_link_stop_refs_on_ilar_delete_trigger
  ON route.infrastructure_link_along_route
  IS 'Trigger to verify the infra link <-> scheduled stop point references.
      Deleting an infrastructure_link_along_route -row can break the route consistency such that a stop point
      might be left on an infra link which is not anymore part of its journey pattern''s route.';
...
COMMENT ON TRIGGER queue_verify_infra_link_stop_refs_on_ssp_update_trigger
  ...
  IS 'Trigger to verify the infra link <-> scheduled stop point references.
      ...
      3. The updated stop point instance may have "revealed" a lower priority stop point (by change of validity time),
         which may make the route and journey pattern conflict in the ways described above with regards to the revealed
         stop point.
...
COMMENT ON TRIGGER queue_verify_infra_link_stop_refs_on_ssp_delete_trigger
  ...
  IS 'Trigger to verify the infra link <-> scheduled stop point references.
      ...
      1. The updated stop point instance may have "revealed" a lower priority stop point, which may make the route and
...
COMMENT ON TRIGGER queue_verify_infra_link_stop_refs_on_jp_update_trigger
  ON journey_pattern.journey_pattern
  IS 'Trigger to verify the infra link <-> scheduled stop point references.
      Updating a journey_pattern row can break the route consistency in the way that the updated journey pattern is
      moved to a route, such that
