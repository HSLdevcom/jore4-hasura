...
ALTER TABLE internal_service_pattern.scheduled_stop_point
  ALTER COLUMN validity_start TYPE timestamp with time zone USING (validity_start AT TIME ZONE 'Europe/Helsinki')::timestamp with time zone;
ALTER TABLE internal_service_pattern.scheduled_stop_point
  ALTER COLUMN validity_end TYPE timestamp with time zone USING ((validity_end AT TIME ZONE 'Europe/Helsinki')::timestamp with time zone + INTERVAL '23:59:59');
...
DROP FUNCTION internal_utils.prev_day(
  bound date
);

DROP FUNCTION internal_utils.next_day(
  bound date
);
