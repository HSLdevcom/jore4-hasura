...
CREATE FUNCTION internal_service_pattern.get_scheduled_stop_points_with_new(
  ...
  RETURNS SETOF internal_service_pattern.scheduled_stop_point
  STABLE
  LANGUAGE plpgsql
AS
$$
BEGIN
  IF new_scheduled_stop_point_id IS NULL THEN
    RETURN QUERY
      SELECT * FROM internal_service_pattern.scheduled_stop_point ssp
  ...
  ELSE
    RETURN QUERY
      ...
      UNION ALL
      SELECT * FROM (
                      VALUES	(new_scheduled_stop_point_id, NULL::GEOGRAPHY(pointz, 4326), new_located_on_infrastructure_link_id, new_direction, new_label, new_validity_start, new_validity_end, NULL::INTEGER)
                    ) AS new_scheduled_stop_point (scheduled_stop_point_id, measured_location, located_on_infrastructure_link_id, direction, "label", validity_start, validity_end, priority);
  END IF;
END;
$$;
