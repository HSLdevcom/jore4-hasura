...
-- journey pattern / route only once. Since constraint triggers can only be specified on row-level, this needs to be
-- done such that the temp table is emptied after the first check, which reduces the same transaction's following
-- trigger runs to no-ops.
--
