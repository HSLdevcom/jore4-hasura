...
ALTER TABLE passing_times.timetabled_passing_time
  ADD CONSTRAINT departure_not_after_arrival CHECK (
...
IS 'Trigger to queue validation of passing times <-> stop point sequences on passing time update.
...
IS 'Trigger to queue validation of passing times <-> stop point sequences on passing time insert.
    Actual validation if triggered later by deferred validate_passing_times_sequence_trigger() trigger';

DROP TRIGGER IF EXISTS validate_passing_times_sequence_trigger ON passing_times.timetabled_passing_time;
CREATE CONSTRAINT TRIGGER validate_passing_times_sequence_trigger AFTER UPDATE OR INSERT ON passing_times.timetabled_passing_time DEFERRABLE INITIALLY DEFERRED FOR EACH ROW WHEN ((NOT passing_times.passing_times_sequence_already_validated())) EXECUTE FUNCTION passing_times.validate_passing_time_sequences();
