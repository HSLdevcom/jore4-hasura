...
CREATE TABLE service_calendar.substitute_operating_day (
  ...
  begin_time interval,
  end_time interval,
  begin_datetime timestamp GENERATED ALWAYS AS (begin_time + superseded_date) STORED,
  end_datetime timestamp GENERATED ALWAYS AS (end_time + superseded_date) STORED
);
...
-- Prevent multiple reference dates for a pair consisting of type-of-line and
-- superseded date. However, allow one reference date and one "no traffic"
-- definition per one superseded date.
CREATE UNIQUE INDEX substitute_operating_day_uniqueness
  ON service_calendar.substitute_operating_day
  USING btree (type_of_line, superseded_date, no_traffic);
