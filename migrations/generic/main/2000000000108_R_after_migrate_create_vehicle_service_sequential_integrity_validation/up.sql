CREATE OR REPLACE FUNCTION timetables.get_vehicle_service_timing_data(vehicle_service_ids uuid[])
  RETURNS TABLE(
...
AS $$
  ...
  WITH timing_data AS (
    ...
    FROM timetables.vehicle_journey journey
    JOIN timetables.timetabled_passing_time tpt USING (vehicle_journey_id)
    JOIN timetables.block block USING (block_id)
    JOIN timetables.vehicle_service service USING (vehicle_service_id)
...
$$;
COMMENT ON FUNCTION timetables.get_vehicle_service_timing_data(vehicle_service_ids uuid[])
...
CREATE OR REPLACE FUNCTION timetables.validate_service_sequential_integrity() RETURNS void
  LANGUAGE plpgsql
AS $$
  DECLARE
    error_message TEXT;
  BEGIN
    -- RAISE NOTICE 'timetables.validate_service_sequential_integrity()';
    ...
    WITH modified_vehicle_service_ids AS (
      ...
      JOIN timetables.vehicle_journey USING (vehicle_journey_id)
      FULL OUTER JOIN modified_block USING (block_id)
      JOIN timetables.block USING (block_id)
    ...
    timing_data AS (
      SELECT * FROM timetables.get_vehicle_service_timing_data(
  ...
  END
$$;
COMMENT ON FUNCTION timetables.validate_service_sequential_integrity()
IS 'Performs validation of sequential integrity on modified vehicle_services:
    ...
    The timing data is collected with timetables.get_vehicle_service_timing_data function.';
