...
CREATE OR REPLACE FUNCTION timetables.queue_validation_by_vsf_id() RETURNS trigger
  LANGUAGE plpgsql
  AS $$
  BEGIN
    -- RAISE LOG 'timetables.queue_validation_by_vsf_id()';
  ...
  END;
  $$;
COMMENT ON FUNCTION timetables.queue_validation_by_vsf_id()
IS 'Queue modified vehicle schedule frames for validation which is performed at the end of transaction.';

CREATE OR REPLACE FUNCTION timetables.queue_validation_by_vs_id() RETURNS trigger
  LANGUAGE plpgsql
  AS $$
  BEGIN
    -- RAISE LOG 'timetables.queue_validation_by_vs_id()';
  ...
  END;
  $$;
COMMENT ON FUNCTION timetables.queue_validation_by_vs_id()
IS 'Queue modified vehicle services for validation which is performed at the end of transaction.';

CREATE OR REPLACE FUNCTION timetables.queue_validation_by_block_id() RETURNS trigger
  LANGUAGE plpgsql
  AS $$
  BEGIN
    -- RAISE LOG 'timetables.queue_validation_by_block_id()';
  ...
  END;
  $$;
COMMENT ON FUNCTION timetables.queue_validation_by_block_id()
IS 'Queue modified vehicle service blocks for validation which is performed at the end of transaction.';

CREATE OR REPLACE FUNCTION timetables.queue_validation_by_vj_id() RETURNS trigger
  LANGUAGE plpgsql
  AS $$
  BEGIN
    -- RAISE LOG 'timetables.queue_validation_by_vj_id()';
  ...
  END;
  $$;
COMMENT ON FUNCTION timetables.queue_validation_by_vj_id()
IS 'Queue modified vehicle journeys for validation which is performed at the end of transaction.';

CREATE OR REPLACE FUNCTION timetables.queue_validation_by_jpr_id() RETURNS trigger
  LANGUAGE plpgsql
  AS $$
  BEGIN
    -- RAISE LOG 'timetables.queue_validation_by_jpr_id()';
  ...
  END;
  $$;
COMMENT ON FUNCTION timetables.queue_validation_by_jpr_id()
...
DROP TRIGGER IF EXISTS queue_vsf_validation_on_insert_trigger ON timetables.vehicle_schedule_frame;
CREATE TRIGGER queue_vsf_validation_on_insert_trigger
  AFTER INSERT ON timetables.vehicle_schedule_frame
  REFERENCING NEW TABLE AS modified_table
  FOR EACH STATEMENT
  EXECUTE FUNCTION timetables.queue_validation_by_vsf_id();
COMMENT ON TRIGGER queue_vsf_validation_on_insert_trigger ON timetables.vehicle_schedule_frame
...
DROP TRIGGER IF EXISTS queue_vsf_validation_on_update_trigger ON timetables.vehicle_schedule_frame;
CREATE TRIGGER queue_vsf_validation_on_update_trigger
  AFTER UPDATE ON timetables.vehicle_schedule_frame
  REFERENCING NEW TABLE AS modified_table
  FOR EACH STATEMENT
  EXECUTE FUNCTION timetables.queue_validation_by_vsf_id();
COMMENT ON TRIGGER queue_vsf_validation_on_update_trigger ON timetables.vehicle_schedule_frame
IS 'Trigger for queuing updated vehicle schedule frames for later validation.
Actual validation is performed at the end of transaction by execute_queued_validations().';

-- vehicle_service:
DROP TRIGGER IF EXISTS queue_vs_validation_on_update_trigger ON timetables.vehicle_service;
CREATE TRIGGER queue_vs_validation_on_update_trigger
  AFTER UPDATE ON timetables.vehicle_service
  REFERENCING NEW TABLE AS modified_table
  FOR EACH STATEMENT
  EXECUTE FUNCTION timetables.queue_validation_by_vs_id();
COMMENT ON TRIGGER queue_vs_validation_on_update_trigger ON timetables.vehicle_service
IS 'Trigger for queuing updated vehicle services for later validation.
Actual validation is performed at the end of transaction by execute_queued_validations().';

-- block:
DROP TRIGGER IF EXISTS queue_block_validation_on_update_trigger ON timetables.block;
CREATE TRIGGER queue_block_validation_on_update_trigger
  AFTER UPDATE ON timetables.block
  REFERENCING NEW TABLE AS modified_table
  FOR EACH STATEMENT
  EXECUTE FUNCTION timetables.queue_validation_by_block_id();
COMMENT ON TRIGGER queue_block_validation_on_update_trigger ON timetables.block
IS 'Trigger for queuing updated vehicle service blocks for later validation.
Actual validation is performed at the end of transaction by execute_queued_validations().';

-- vehicle_journey:
DROP TRIGGER IF EXISTS queue_vj_validation_on_insert_trigger ON timetables.vehicle_journey;
CREATE TRIGGER queue_vj_validation_on_insert_trigger
  AFTER INSERT ON timetables.vehicle_journey
  REFERENCING NEW TABLE AS modified_table
  FOR EACH STATEMENT
  EXECUTE FUNCTION timetables.queue_validation_by_vj_id();
COMMENT ON TRIGGER queue_vj_validation_on_insert_trigger ON timetables.vehicle_journey
IS 'Trigger for queuing inserted vehicle journeys for later validation.
Actual validation is performed at the end of transaction by execute_queued_validations().';

DROP TRIGGER IF EXISTS queue_vj_validation_on_update_trigger ON timetables.vehicle_journey;
CREATE TRIGGER queue_vj_validation_on_update_trigger
  AFTER UPDATE ON timetables.vehicle_journey
  REFERENCING NEW TABLE AS modified_table
  FOR EACH STATEMENT
  EXECUTE FUNCTION timetables.queue_validation_by_vj_id();
COMMENT ON TRIGGER queue_vj_validation_on_update_trigger ON timetables.vehicle_journey
IS 'Trigger for queuing updated vehicle journeys for later validation.
Actual validation is performed at the end of transaction by execute_queued_validations().';

-- journey_pattern_ref:
DROP TRIGGER IF EXISTS queue_jpr_validation_on_update_trigger ON timetables.journey_pattern_ref;
CREATE TRIGGER queue_jpr_validation_on_update_trigger
  AFTER UPDATE ON timetables.journey_pattern_ref
  REFERENCING NEW TABLE AS modified_table
  FOR EACH STATEMENT
  EXECUTE FUNCTION timetables.queue_validation_by_jpr_id();
COMMENT ON TRIGGER queue_jpr_validation_on_update_trigger ON timetables.journey_pattern_ref
IS 'Trigger for queuing updated journey pattern refs for later validation.
Actual validation is performed at the end of transaction by execute_queued_validations().';

DROP TRIGGER IF EXISTS queue_jpr_validation_on_insert_trigger ON timetables.journey_pattern_ref;
CREATE TRIGGER queue_jpr_validation_on_insert_trigger
  AFTER INSERT ON timetables.journey_pattern_ref
  REFERENCING NEW TABLE AS modified_table
  FOR EACH STATEMENT
  EXECUTE FUNCTION timetables.queue_validation_by_jpr_id();
COMMENT ON TRIGGER queue_jpr_validation_on_insert_trigger ON timetables.journey_pattern_ref
...
DROP TRIGGER IF EXISTS queue_vj_validation_on_pt_update_trigger ON timetables.timetabled_passing_time;
CREATE TRIGGER queue_vj_validation_on_pt_update_trigger
  AFTER UPDATE ON timetables.timetabled_passing_time
  REFERENCING NEW TABLE AS modified_table
  FOR EACH STATEMENT
  EXECUTE FUNCTION timetables.queue_validation_by_vj_id();
COMMENT ON TRIGGER queue_vj_validation_on_pt_update_trigger ON timetables.timetabled_passing_time
IS 'Trigger to queue validation of parent vehicle_journey on timetabled_passing_times update.
Actual validation is performed at the end of transaction by execute_queued_validations().';

DROP TRIGGER IF EXISTS queue_vj_validation_on_pt_insert_trigger ON timetables.timetabled_passing_time;
CREATE TRIGGER queue_vj_validation_on_pt_insert_trigger
  AFTER INSERT ON timetables.timetabled_passing_time
  REFERENCING NEW TABLE AS modified_table
  FOR EACH STATEMENT
  EXECUTE FUNCTION timetables.queue_validation_by_vj_id();
COMMENT ON TRIGGER queue_vj_validation_on_pt_insert_trigger ON timetables.timetabled_passing_time
IS 'Trigger to queue validation of parent vehicle_journey on timetabled_passing_times insert.
Actual validation is performed at the end of transaction by execute_queued_validations().';

DROP TRIGGER IF EXISTS queue_vj_validation_on_pt_delete_trigger ON timetables.timetabled_passing_time;
CREATE TRIGGER queue_vj_validation_on_pt_delete_trigger
  AFTER DELETE ON timetables.timetabled_passing_time
  REFERENCING OLD TABLE AS modified_table
  FOR EACH STATEMENT
  EXECUTE FUNCTION timetables.queue_validation_by_vj_id();
COMMENT ON TRIGGER queue_vj_validation_on_pt_delete_trigger ON timetables.timetabled_passing_time
...
DROP TRIGGER IF EXISTS queue_jpr_validation_on_ssp_update_trigger ON timetables.scheduled_stop_point_in_journey_pattern_ref;
CREATE TRIGGER queue_jpr_validation_on_ssp_update_trigger
  AFTER UPDATE ON timetables.scheduled_stop_point_in_journey_pattern_ref
  REFERENCING NEW TABLE AS modified_table
  FOR EACH STATEMENT
  EXECUTE FUNCTION timetables.queue_validation_by_jpr_id();
COMMENT ON TRIGGER queue_jpr_validation_on_ssp_update_trigger ON timetables.scheduled_stop_point_in_journey_pattern_ref
IS 'Trigger to queue validation of parent journey_pattern_ref on scheduled_stop_point_in_journey_pattern_ref update.
Actual validation is performed at the end of transaction by execute_queued_validations().';

DROP TRIGGER IF EXISTS queue_jpr_validation_on_ssp_insert_trigger ON timetables.scheduled_stop_point_in_journey_pattern_ref;
CREATE TRIGGER queue_jpr_validation_on_ssp_insert_trigger
  AFTER INSERT ON timetables.scheduled_stop_point_in_journey_pattern_ref
  REFERENCING NEW TABLE AS modified_table
  FOR EACH STATEMENT
  EXECUTE FUNCTION timetables.queue_validation_by_jpr_id();
COMMENT ON TRIGGER queue_jpr_validation_on_ssp_insert_trigger ON timetables.scheduled_stop_point_in_journey_pattern_ref
...
DROP TRIGGER IF EXISTS process_queued_validation_on_vsf_trigger ON timetables.vehicle_schedule_frame;
CREATE CONSTRAINT TRIGGER process_queued_validation_on_vsf_trigger
  AFTER INSERT OR UPDATE ON timetables.vehicle_schedule_frame
...
COMMENT ON TRIGGER process_queued_validation_on_vsf_trigger ON timetables.vehicle_schedule_frame
...
DROP TRIGGER IF EXISTS process_queued_validation_on_vs_trigger ON timetables.vehicle_service;
CREATE CONSTRAINT TRIGGER process_queued_validation_on_vs_trigger
  ...
  WHEN (NOT internal_utils.queued_validations_already_processed())
  EXECUTE FUNCTION internal_utils.execute_queued_validations();
COMMENT ON TRIGGER process_queued_validation_on_vs_trigger ON timetables.vehicle_service
IS 'Trigger to execute queued validations at the end of the transaction that were registered earlier by statement level triggers';

DROP TRIGGER IF EXISTS process_queued_validation_on_block_trigger ON timetables.block;
CREATE CONSTRAINT TRIGGER process_queued_validation_on_block_trigger
  ...
  WHEN (NOT internal_utils.queued_validations_already_processed())
  EXECUTE FUNCTION internal_utils.execute_queued_validations();
COMMENT ON TRIGGER process_queued_validation_on_block_trigger ON timetables.block
IS 'Trigger to execute queued validations at the end of the transaction that were registered earlier by statement level triggers';

DROP TRIGGER IF EXISTS process_queued_validation_on_vj_trigger ON timetables.vehicle_journey;
CREATE CONSTRAINT TRIGGER process_queued_validation_on_vj_trigger
  ...
  AFTER UPDATE OR INSERT OR DELETE ON timetables.vehicle_journey
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW
  WHEN (NOT internal_utils.queued_validations_already_processed())
  EXECUTE FUNCTION internal_utils.execute_queued_validations();
COMMENT ON TRIGGER process_queued_validation_on_vj_trigger ON timetables.vehicle_journey
IS 'Trigger to execute queued validations at the end of the transaction that were registered earlier by statement level triggers';

DROP TRIGGER IF EXISTS process_queued_validation_on_jpr_trigger ON timetables.journey_pattern_ref;
CREATE CONSTRAINT TRIGGER process_queued_validation_on_jpr_trigger
  AFTER UPDATE OR INSERT ON timetables.journey_pattern_ref
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW
  WHEN (NOT internal_utils.queued_validations_already_processed())
  EXECUTE FUNCTION internal_utils.execute_queued_validations();
COMMENT ON TRIGGER process_queued_validation_on_jpr_trigger ON timetables.journey_pattern_ref
IS 'Trigger to execute queued validations at the end of the transaction that were registered earlier by statement level triggers';

DROP TRIGGER IF EXISTS process_queued_validation_on_pt_trigger ON timetables.timetabled_passing_time;
CREATE CONSTRAINT TRIGGER process_queued_validation_on_pt_trigger
  AFTER UPDATE OR INSERT OR DELETE ON timetables.timetabled_passing_time
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW
  WHEN (NOT internal_utils.queued_validations_already_processed())
  EXECUTE FUNCTION internal_utils.execute_queued_validations();
COMMENT ON TRIGGER process_queued_validation_on_pt_trigger ON timetables.timetabled_passing_time
IS 'Trigger to execute queued validations at the end of the transaction that were registered earlier by statement level triggers';

DROP TRIGGER IF EXISTS process_queued_validation_on_ssp_trigger ON timetables.scheduled_stop_point_in_journey_pattern_ref;
CREATE CONSTRAINT TRIGGER process_queued_validation_on_ssp_trigger
  AFTER UPDATE OR INSERT ON timetables.scheduled_stop_point_in_journey_pattern_ref
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW
  WHEN (NOT internal_utils.queued_validations_already_processed())
  EXECUTE FUNCTION internal_utils.execute_queued_validations();
COMMENT ON TRIGGER process_queued_validation_on_ssp_trigger ON timetables.scheduled_stop_point_in_journey_pattern_ref
IS 'Trigger to execute queued validations at the end of the transaction that were registered earlier by statement level triggers';
