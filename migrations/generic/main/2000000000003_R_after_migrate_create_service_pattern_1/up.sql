CREATE OR REPLACE VIEW network.scheduled_stop_points_with_infra_link_data AS (
...
COMMENT ON VIEW network.scheduled_stop_points_with_infra_link_data
IS 'Contains scheduled_stop_points enriched with some infra link data.';

CREATE OR REPLACE FUNCTION network.new_scheduled_stop_point_if_id_given(
...
$$;
COMMENT ON FUNCTION network.new_scheduled_stop_point_if_id_given(new_scheduled_stop_point_id uuid, new_located_on_infrastructure_link_id uuid, new_measured_location public.geography, new_direction text, new_label text, new_validity_start date, new_validity_end date, new_priority integer)
