CREATE OR REPLACE FUNCTION network.find_effective_scheduled_stop_points_in_journey_pattern(
...
WITH unambiguous_sspijp AS (
  ...
  JOIN network.scheduled_stop_point_invariant sspi ON sspi.label = sspijp.scheduled_stop_point_label
  JOIN network.scheduled_stop_point ssp USING (label)
...
JOIN network.scheduled_stop_point_in_journey_pattern USING (journey_pattern_id, scheduled_stop_point_sequence)
$$;
COMMENT ON FUNCTION network.find_effective_scheduled_stop_points_in_journey_pattern(uuid, date, boolean)
IS 'Find effective scheduled stop points in journey/service pattern.';


CREATE OR REPLACE FUNCTION network.find_scheduled_stop_point_locations_in_journey_pattern(
  filter_journey_pattern_id  uuid,
  observation_date           date,
  include_draft_stops        boolean
)
...
SELECT
  ...
  ssp.timing_place_id
FROM network.find_effective_scheduled_stop_points_in_journey_pattern(
  filter_journey_pattern_id,
  observation_date,
  include_draft_stops
) sspijp
JOIN network.scheduled_stop_point ssp ON ssp.scheduled_stop_point_id = sspijp.effective_scheduled_stop_point_id
JOIN infrastructure_network.infrastructure_link il ON il.infrastructure_link_id = ssp.located_on_infrastructure_link_id
$$;
COMMENT ON FUNCTION network.find_scheduled_stop_point_locations_in_journey_pattern(uuid, date, boolean)
IS 'Find location information for scheduled stop points in journey/service pattern.';


CREATE OR REPLACE FUNCTION network.get_distances_between_stop_points_in_journey_patterns(
  journey_pattern_ids  uuid[],
  observation_date     date,
  include_draft_stops  boolean
)
  RETURNS SETOF network.distance_between_stops_calculation
  STABLE
  PARALLEL SAFE
  LANGUAGE sql AS
$$
...
scheduled_stop_point_info AS (
  ...
  JOIN LATERAL (
    SELECT *
    FROM network.find_scheduled_stop_point_locations_in_journey_pattern(jp.journey_pattern_id, observation_date, include_draft_stops)
...
ssp_merged_with_ilar AS ( -- contains recursive term
  ...
  JOIN network.infrastructure_link_along_route ilar USING (route_id, infrastructure_link_id)
...
JOIN network.route USING (route_id)
ORDER BY route_id, journey_pattern_id, stop_interval_sequence
$$;
COMMENT ON FUNCTION network.get_distances_between_stop_points_in_journey_patterns(uuid[], date, boolean)
IS 'Get the distances between scheduled stop points (in metres) for the given journey/service patterns.';


CREATE OR REPLACE FUNCTION network.get_distances_between_stop_points_in_journey_pattern(
  journey_pattern_id   uuid,
  observation_date     date,
  include_draft_stops  boolean
)
...
$$
SELECT *
FROM network.get_distances_between_stop_points_in_journey_patterns(ARRAY[journey_pattern_id], observation_date, include_draft_stops)
$$;
COMMENT ON FUNCTION network.get_distances_between_stop_points_in_journey_pattern(uuid, date, boolean)
IS 'Get the distances between scheduled stop points (in metres) for the given journey/service pattern.';


CREATE OR REPLACE FUNCTION network.get_distances_between_stop_points_by_routes(
  route_ids         uuid[],
  observation_date  date
)
  RETURNS SETOF network.distance_between_stops_calculation
...
JOIN LATERAL (
  SELECT *
  FROM network.get_distances_between_stop_points_in_journey_patterns(ids.journey_pattern_ids, observation_date, false)
) stop_interval_length USING (route_id)
ORDER BY journey_pattern_id ASC, stop_interval_sequence ASC
$$;
COMMENT ON FUNCTION network.get_distances_between_stop_points_by_routes(uuid[], date)
