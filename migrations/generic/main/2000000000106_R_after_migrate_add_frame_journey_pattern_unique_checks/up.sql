...
CREATE OR REPLACE FUNCTION timetables.get_overlapping_schedules(
...
AS $$
  ...
  vehicle_schedule_frame_journey_patterns AS (
    SELECT DISTINCT
      ...
      day_of_week,
      priority
    FROM timetables.journey_patterns_in_vehicle_service
    JOIN timetables.vehicle_service USING (vehicle_service_id)
    JOIN timetables.vehicle_schedule_frame USING (vehicle_schedule_frame_id)
    JOIN timetables.day_type_active_on_day_of_week USING (day_type_id)
    JOIN journey_patterns_to_check USING (journey_pattern_id)
    WHERE (
      ignore_priority = true OR
      priority < internal_utils.const_priority_draft() -- The restrictions should not apply for Draft and Staging priorities.
    )
  ),
...
$$;
COMMENT ON FUNCTION timetables.get_overlapping_schedules(
  filter_vehicle_schedule_frame_ids uuid[],
  filter_journey_pattern_ref_ids uuid[],
...
CREATE OR REPLACE FUNCTION timetables.validate_queued_schedules_uniqueness() RETURNS VOID
  LANGUAGE plpgsql
AS $$
DECLARE
  ...
  error_message TEXT;
BEGIN
  -- RAISE NOTICE 'timetables.validate_queued_schedules_uniqueness()';
  ...
  SELECT * FROM timetables.get_overlapping_schedules(
...
END;
$$;
COMMENT ON FUNCTION timetables.validate_queued_schedules_uniqueness()
