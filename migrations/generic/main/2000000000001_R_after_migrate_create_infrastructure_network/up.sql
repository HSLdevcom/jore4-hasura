    ...
    FROM network.infrastructure_link_along_route
           JOIN infrastructure_network.infrastructure_link
                ON infrastructure_network.infrastructure_link.infrastructure_link_id =
                   network.infrastructure_link_along_route.infrastructure_link_id
    WHERE network.infrastructure_link_along_route.infrastructure_link_id = NEW.infrastructure_link_id
      AND (
      -- NB: NEW.direction = 'bidirectional' allows both traversal directions
        (network.infrastructure_link_along_route.is_traversal_forwards = TRUE AND NEW.direction = 'backward') OR
        (network.infrastructure_link_along_route.is_traversal_forwards = FALSE AND NEW.direction = 'forward')
      )
    )
  THEN
...
CREATE OR REPLACE FUNCTION network.check_infrastructure_link_scheduled_stop_points_direction_trigg() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  IF EXISTS(
    ...
    WHERE network.scheduled_stop_point.located_on_infrastructure_link_id = NEW.infrastructure_link_id
    AND (
      (network.scheduled_stop_point.direction = 'forward' AND NEW.direction = 'backward') OR
      (network.scheduled_stop_point.direction = 'backward' AND NEW.direction = 'forward') OR
      (network.scheduled_stop_point.direction = 'bidirectional' AND NEW.direction != 'bidirectional')
    )
  )
  THEN
...
CREATE CONSTRAINT TRIGGER check_infrastructure_link_scheduled_stop_points_direction_trigg AFTER UPDATE ON infrastructure_network.infrastructure_link DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION network.check_infrastructure_link_scheduled_stop_points_direction_trigg();
