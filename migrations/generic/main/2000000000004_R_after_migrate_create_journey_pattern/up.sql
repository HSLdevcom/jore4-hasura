CREATE OR REPLACE FUNCTION network.maximum_priority_validity_spans(
  entity_type text,
...
WITH RECURSIVE
  ...
  entity AS (
    ...
    SELECT ssp.scheduled_stop_point_id AS id,
           ...
           ssp.priority
    FROM (
      SELECT * FROM network.scheduled_stop_points_with_infra_link_data ssp
        WHERE replace_scheduled_stop_point_id IS DISTINCT FROM ssp.scheduled_stop_point_id
      UNION ALL
        SELECT * FROM network.new_scheduled_stop_point_if_id_given(
    ...
    WHERE entity_type = 'scheduled_stop_point'
      ...
      AND (EXISTS(
             SELECT 1
             FROM network.scheduled_stop_point_in_journey_pattern sspijp
                    JOIN network.journey_pattern jp ON jp.journey_pattern_id = sspijp.journey_pattern_id
                    JOIN network.route r
...
CREATE OR REPLACE FUNCTION network.get_broken_route_check_filters(filter_route_ids uuid[]) RETURNS TABLE(labels text[], validity_start date, validity_end date)
    LANGUAGE sql STABLE PARALLEL SAFE
    AS $$
WITH RECURSIVE
  route_param AS (
...
$$;
COMMENT ON FUNCTION network.get_broken_route_check_filters(filter_route_ids uuid[]) IS 'Gather the filter parameters (route labels and validity range to check) for the broken route check.';

CREATE OR REPLACE FUNCTION network.get_broken_route_journey_patterns(
...
RETURNS SETOF network.journey_pattern
    LANGUAGE sql STABLE PARALLEL SAFE
    AS $$
WITH RECURSIVE
  new_ssp_param AS (
  ...
  filter_route AS (
    ...
    FROM network.get_broken_route_check_filters(filter_route_ids)
  ),
  ...
  prioritized_route AS (
    ...
    FROM network.route r
           JOIN network.maximum_priority_validity_spans(
      'route',
  ...
  priority_validity_spans AS (
    SELECT * FROM network.maximum_priority_validity_spans(
  ...
  prioritized_ssp_with_new AS (
    ...
    FROM (
      SELECT
        ...
        FROM network.scheduled_stop_points_with_infra_link_data ssp
      ...
      UNION ALL
        SELECT
           ...
           priority_validity_spans.validity_start AS priority_span_validity_start,
           priority_validity_spans.validity_end AS priority_span_validity_end
        FROM network.new_scheduled_stop_point_if_id_given(
          (SELECT new_scheduled_stop_point_id FROM new_ssp_param),
          new_located_on_infrastructure_link_id,
          new_measured_location,
          new_direction,
  ...
  sspijp_ilar_combos AS (
    ...
    FROM (
           ...
           FROM network.route r
                  JOIN network.journey_pattern jp ON jp.on_route_id = r.route_id
                  JOIN network.scheduled_stop_point_in_journey_pattern sspijp
           ...
           JOIN network.infrastructure_link_along_route ilar
...
CREATE OR REPLACE FUNCTION network.check_infra_link_stop_refs_with_new_scheduled_stop_point(
...
WITH filter_route_ids AS (
  ...
  FROM network.route r
         LEFT JOIN network.journey_pattern jp ON jp.on_route_id = r.route_id
         LEFT JOIN network.scheduled_stop_point_in_journey_pattern sspijp
                   ON sspijp.journey_pattern_id = jp.journey_pattern_id
         LEFT JOIN network.scheduled_stop_point ssp
                   ON ssp.label = sspijp.scheduled_stop_point_label
...
$$;
COMMENT ON FUNCTION network.check_infra_link_stop_refs_with_new_scheduled_stop_point(replace_scheduled_stop_point_id uuid, new_located_on_infrastructure_link_id uuid, new_measured_location public.geography, new_direction text, new_label text, new_validity_start date, new_validity_end date, new_priority integer)
...
CREATE OR REPLACE FUNCTION network.create_verify_infra_link_stop_refs_queue_temp_table() RETURNS void
  ...
  $$;
COMMENT ON FUNCTION network.create_verify_infra_link_stop_refs_queue_temp_table() IS 'Create the temp table used to enqueue verification of the changed routes from
  statement-level triggers';

CREATE OR REPLACE FUNCTION network.infra_link_stop_refs_already_verified() RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
...
END
$$;
COMMENT ON FUNCTION network.infra_link_stop_refs_already_verified() IS 'Keep track of whether the infra link <-> stop ref verification has already been performed in this transaction';

CREATE OR REPLACE FUNCTION network.queue_verify_infra_link_stop_refs_by_new_journey_pattern_id() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  -- RAISE NOTICE 'network.queue_verify_infra_link_stop_refs_by_new_journey_pattern_id()';

  PERFORM network.create_verify_infra_link_stop_refs_queue_temp_table();

  INSERT INTO updated_route (route_id)
  SELECT r.route_id
  FROM new_table
         JOIN network.journey_pattern jp ON jp.journey_pattern_id = new_table.journey_pattern_id
         JOIN network.route r ON r.route_id = jp.on_route_id
...
CREATE OR REPLACE FUNCTION network.queue_verify_infra_link_stop_refs_by_new_ssp_label() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  -- RAISE NOTICE 'network.queue_verify_infra_link_stop_refs_by_new_ssp_label()';
  ...
  SELECT r.route_id
  FROM new_table
         JOIN network.route r ON EXISTS(
    SELECT 1
    FROM network.scheduled_stop_point_in_journey_pattern sspijp
           JOIN network.journey_pattern jp ON jp.journey_pattern_id = sspijp.journey_pattern_id
...
CREATE OR REPLACE FUNCTION network.queue_verify_infra_link_stop_refs_by_old_route_id() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  -- RAISE NOTICE 'network.queue_verify_infra_link_stop_refs_by_old_route_id()';
  ...
  SELECT r.route_id
  FROM old_table
         JOIN network.route r ON r.route_id = old_table.route_id
  ON CONFLICT DO NOTHING;
...
CREATE OR REPLACE FUNCTION network.queue_verify_infra_link_stop_refs_by_old_route_label() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  -- RAISE NOTICE 'network.queue_verify_infra_link_stop_refs_by_old_route_label()';
...
CREATE OR REPLACE FUNCTION network.queue_verify_infra_link_stop_refs_by_old_ssp_label() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  -- RAISE NOTICE 'network.queue_verify_infra_link_stop_refs_by_old_ssp_label()';
...
CREATE OR REPLACE FUNCTION network.truncate_scheduled_stop_point_in_journey_pattern() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  --RAISE NOTICE 'network.truncate_scheduled_stop_point_in_journey_pattern()';

  IF (SELECT COUNT(*) FROM network.scheduled_stop_point_in_journey_pattern) > 0 THEN
    RAISE WARNING 'TRUNCATING network.scheduled_stop_point_in_journey_pattern to ensure data consistency';
    TRUNCATE network.scheduled_stop_point_in_journey_pattern CASCADE;
  END IF;

  RETURN NULL;
END;
$$;
COMMENT ON FUNCTION network.truncate_scheduled_stop_point_in_journey_pattern() IS 'Truncate the scheduled_stop_point_in_journey_pattern if it contains any rows. It must not be truncated if it
...
END;
$$;
COMMENT ON FUNCTION network.verify_infra_link_stop_refs() IS 'Perform verification of all queued route entries.
   Raise an exception if the specified journey pattern''s / route''s links and stop points do not
   correspond to each other.';

CREATE OR REPLACE FUNCTION network.verify_route_journey_pattern_refs(filter_journey_pattern_id uuid, filter_route_id uuid) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
  IF NOT network.check_route_journey_pattern_refs(
...
END;
$$;
COMMENT ON FUNCTION network.verify_route_journey_pattern_refs(filter_journey_pattern_id uuid, filter_route_id uuid) IS 'Raise an exception if the specified journey pattern''s / route''s links and stop points do not
   correspond to each other.';


DROP TRIGGER IF EXISTS queue_verify_infra_link_stop_refs_on_jp_update_trigger ON network.journey_pattern;
CREATE TRIGGER queue_verify_infra_link_stop_refs_on_jp_update_trigger AFTER UPDATE ON network.journey_pattern REFERENCING NEW TABLE AS new_table FOR EACH STATEMENT EXECUTE FUNCTION network.queue_verify_infra_link_stop_refs_by_new_journey_pattern_id();
COMMENT ON TRIGGER queue_verify_infra_link_stop_refs_on_jp_update_trigger ON network.journey_pattern
...
DROP TRIGGER IF EXISTS queue_verify_infra_link_stop_refs_on_sspijp_insert_trigger ON network.scheduled_stop_point_in_journey_pattern;
CREATE TRIGGER queue_verify_infra_link_stop_refs_on_sspijp_insert_trigger AFTER INSERT ON network.scheduled_stop_point_in_journey_pattern REFERENCING NEW TABLE AS new_table FOR EACH STATEMENT EXECUTE FUNCTION network.queue_verify_infra_link_stop_refs_by_new_journey_pattern_id();
COMMENT ON TRIGGER queue_verify_infra_link_stop_refs_on_sspijp_insert_trigger ON network.scheduled_stop_point_in_journey_pattern
IS 'Trigger to verify the infra link <-> scheduled stop point references.
...
DROP TRIGGER IF EXISTS queue_verify_infra_link_stop_refs_on_sspijp_update_trigger ON network.scheduled_stop_point_in_journey_pattern;
CREATE TRIGGER queue_verify_infra_link_stop_refs_on_sspijp_update_trigger AFTER UPDATE ON network.scheduled_stop_point_in_journey_pattern REFERENCING NEW TABLE AS new_table FOR EACH STATEMENT EXECUTE FUNCTION network.queue_verify_infra_link_stop_refs_by_new_journey_pattern_id();
COMMENT ON TRIGGER queue_verify_infra_link_stop_refs_on_sspijp_update_trigger ON network.scheduled_stop_point_in_journey_pattern
IS 'Trigger to verify the infra link <-> scheduled stop point references.
      ...
      3. The updated stop point might not have any instances whose validity time would at all overlap with the journey
         pattern''s route''s validity time ("ghost stop").';
DROP TRIGGER IF EXISTS verify_infra_link_stop_refs_on_journey_pattern_trigger ON network.journey_pattern;
CREATE CONSTRAINT TRIGGER verify_infra_link_stop_refs_on_journey_pattern_trigger AFTER UPDATE ON network.journey_pattern DEFERRABLE INITIALLY DEFERRED FOR EACH ROW WHEN ((NOT network.infra_link_stop_refs_already_verified())) EXECUTE FUNCTION network.verify_infra_link_stop_refs();
COMMENT ON TRIGGER verify_infra_link_stop_refs_on_journey_pattern_trigger ON network.journey_pattern
IS 'Trigger to verify the infra link <-> stop references after a delete on the
   ...
   level trigger.';
DROP TRIGGER IF EXISTS verify_infra_link_stop_refs_on_sspijp_trigger ON network.scheduled_stop_point_in_journey_pattern;
CREATE CONSTRAINT TRIGGER verify_infra_link_stop_refs_on_sspijp_trigger AFTER INSERT OR UPDATE ON network.scheduled_stop_point_in_journey_pattern DEFERRABLE INITIALLY DEFERRED FOR EACH ROW WHEN ((NOT network.infra_link_stop_refs_already_verified())) EXECUTE FUNCTION network.verify_infra_link_stop_refs();
COMMENT ON TRIGGER verify_infra_link_stop_refs_on_sspijp_trigger ON network.scheduled_stop_point_in_journey_pattern
...
ALTER TABLE network.scheduled_stop_point_in_journey_pattern
...
CREATE OR REPLACE FUNCTION network.scheduled_stop_point_has_timing_place_if_used_as_timing_point() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  ...
  IF EXISTS(
    SELECT 1
      FROM network.scheduled_stop_point_in_journey_pattern as sspijp
      JOIN network.scheduled_stop_point AS ssp ON sspijp.scheduled_stop_point_label = ssp.label
...
END;
$$;
COMMENT ON FUNCTION network.scheduled_stop_point_has_timing_place_if_used_as_timing_point()
...
DROP TRIGGER IF EXISTS scheduled_stop_point_has_timing_place_if_used_as_timing_point_trigger ON network.scheduled_stop_point_in_journey_pattern;
CREATE CONSTRAINT TRIGGER scheduled_stop_point_has_timing_place_if_used_as_timing_point_trigger AFTER INSERT OR UPDATE ON network.scheduled_stop_point_in_journey_pattern DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION network.scheduled_stop_point_has_timing_place_if_used_as_timing_point();

DROP TRIGGER IF EXISTS scheduled_stop_point_has_timing_place_if_used_as_timing_point_trigger ON network.scheduled_stop_point;
CREATE CONSTRAINT TRIGGER scheduled_stop_point_has_timing_place_if_used_as_timing_point_trigger AFTER INSERT OR UPDATE ON network.scheduled_stop_point DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION network.scheduled_stop_point_has_timing_place_if_used_as_timing_point();
