ALTER TABLE timetables.journey_patterns_in_vehicle_service
...
CREATE OR REPLACE FUNCTION timetables.refresh_journey_patterns_in_vehicle_service() RETURNS void
  LANGUAGE plpgsql
...
BEGIN
  ...
  UPDATE timetables.journey_patterns_in_vehicle_service
  SET reference_count = 0;

  -- Step 2: upsert new entries.
  INSERT INTO timetables.journey_patterns_in_vehicle_service (journey_pattern_id, vehicle_service_id, reference_count)
    SELECT DISTINCT journey_pattern_id, vehicle_service_id, COUNT(journey_pattern_ref_id) AS ref_count
    FROM timetables.vehicle_service
    JOIN timetables.block USING (vehicle_service_id)
    JOIN timetables.vehicle_journey USING (block_id)
    JOIN timetables.journey_pattern_ref USING (journey_pattern_ref_id)
  ...
  DELETE FROM timetables.journey_patterns_in_vehicle_service
  WHERE reference_count = 0;
END;
$$;
COMMENT ON FUNCTION timetables.refresh_journey_patterns_in_vehicle_service()
