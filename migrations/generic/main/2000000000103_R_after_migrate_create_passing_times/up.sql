ALTER TABLE timetables.timetabled_passing_time
...
CREATE OR REPLACE FUNCTION timetables.get_passing_time_order_validity_data(filter_vehicle_journey_ids uuid[], filter_journey_pattern_ref_ids uuid[])
RETURNS TABLE(
...
WITH RECURSIVE
  ...
  passing_time_sequence_combos AS (
    ...
    JOIN timetables.scheduled_stop_point_in_journey_pattern_ref ssp USING (scheduled_stop_point_in_journey_pattern_ref_id)
    JOIN timetables.vehicle_journey vj USING (vehicle_journey_id)
...
$$;
COMMENT ON FUNCTION timetables.get_passing_time_order_validity_data(filter_vehicle_journey_ids uuid[], filter_journey_pattern_ref_ids uuid[]) IS '
...
CREATE OR REPLACE FUNCTION timetables.validate_passing_time_sequences() RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
  row_validation_data RECORD;
BEGIN
  -- RAISE NOTICE 'timetables.validate_passing_time_sequences()';
  ...
  FOR row_validation_data IN (
    ...
    SELECT *
    FROM timetables.get_passing_time_order_validity_data(
      (SELECT ids FROM filter_vehicle_journey_ids),
      (SELECT ids FROM filter_journey_pattern_ref_ids)
    )

    JOIN timetables.timetabled_passing_time USING (vehicle_journey_id)
  )
  LOOP
...
END;
$$;
COMMENT ON FUNCTION timetables.validate_passing_time_sequences()
