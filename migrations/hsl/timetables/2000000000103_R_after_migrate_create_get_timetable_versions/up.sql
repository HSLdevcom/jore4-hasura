...
CREATE OR REPLACE FUNCTION vehicle_service.get_timetables_and_substitute_operating_days_by_journey_pattern_ids(journey_pattern_ids uuid[], begin_date date, end_date date)
 ...
 STABLE
AS $function$
  SELECT result.* FROM (SELECT unnest(journey_pattern_ids) AS jp_id) as subquery
    CROSS JOIN LATERAL vehicle_service.get_timetables_and_substitute_operating_days(subquery.jp_id, begin_date, end_date) result
$function$
;
...
AS $FUNCTION$
  BEGIN
    CREATE TEMP TABLE IF NOT EXISTS table_holder ON COMMIT DELETE ROWS AS
  ...
  WHERE priority = 25 
  AND validity_start <= observation_date 
  AND validity_end >= observation_date;
  
  UPDATE table_holder AS subquery SET in_effect = true
  WHERE priority = 23 
  AND validity_start <= observation_date 
  AND validity_end >= observation_date 
  AND NOT EXISTS (SELECT * FROM table_holder WHERE day_type_id = subquery.day_type_id AND in_effect = true);
