...
SELECT DISTINCT vsf.vehicle_schedule_frame_id, null::uuid, vsf.validity_start, vsf.validity_end, vsf.priority, false, dt.day_type_id 
	...
	WHERE jpr.journey_pattern_id = journey_pattern_uuid AND vsf.validity_end >= begin_date AND vsf.validity_start <= end_date
union 
...
HAVING count(label) = 1)
	FROM journey_pattern.journey_pattern_ref jpr 
		...
		WHERE jpr.journey_pattern_id = journey_pattern_uuid AND sodblt.superseded_date >= begin_date AND sodblt.superseded_date <= end_date
$function$
;

COMMENT ON FUNCTION vehicle_service.get_timetables_and_substitute_operating_days IS 'Uses return_value.timetable_version as return value. Returns all the corresponding data from 
...
COMMENT ON FUNCTION vehicle_service.get_timetables_and_substitute_operating_days_by_journey_pattern_ids IS 'Helper function to call vehicle_service.get_timetables_and_substitute_operating_days
for each given journey_pattern.';
...
AS $FUNCTION$
	DECLARE 
	record return_value.timetable_version;
	BEGIN
...
COMMENT ON FUNCTION vehicle_service.get_timetables_and_substitute_operating_days_by_journey_pattern_ids IS 'Uses the return_value.timetable_version as the return value. This function will get
