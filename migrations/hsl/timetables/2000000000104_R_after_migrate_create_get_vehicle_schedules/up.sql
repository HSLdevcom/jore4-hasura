CREATE OR REPLACE FUNCTION internal_utils.get_substitute_operating_period_begin_date(substitute_operating_period_uuid uuid)
RETURNS date
LANGUAGE SQL
  STABLE
AS $$
  ...
  WHERE sodblt.substitute_operating_period_id = substitute_operating_period_uuid;
$$
;

COMMENT ON FUNCTION internal_utils.get_substitute_operating_period_begin_date(substitute_operating_period_uuid uuid)
IS 'Returns the begin date of a substitute operating period that consists of substitute_operating_day_by_line_types
(superseded_date).
';

CREATE OR REPLACE FUNCTION internal_utils.get_substitute_operating_period_end_date(substitute_operating_period_uuid uuid)
RETURNS date
LANGUAGE SQL
  STABLE
AS $$
...
COMMENT ON FUNCTION internal_utils.get_substitute_operating_period_end_date(substitute_operating_period_uuid uuid)
...
(
  SELECT DISTINCT vj.vehicle_journey_id,
    internal_utils.get_substitute_operating_period_begin_date(sodblt.substitute_operating_period_id),
    internal_utils.get_substitute_operating_period_end_date(sodblt.substitute_operating_period_id),
