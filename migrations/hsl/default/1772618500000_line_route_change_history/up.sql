...
COMMENT ON COLUMN route.line_change_history.route_direction IS
  'Label of the HSL route that was changed.';
...
CREATE MATERIALIZED VIEW route.tmp_stopPointsOnRoute AS
...
CREATE INDEX tmp_idx_stopPointsOnRoute_route_id ON route.tmp_stopPointsOnRoute(routeId);

CREATE MATERIALIZED VIEW route.tmp_estimatedRouteLengthInMeters AS
SELECT ilar.route_id AS routeId, sum(il.estimated_length_in_metres) AS length
FROM route.infrastructure_link_along_route AS ilar
INNER JOIN infrastructure_network.infrastructure_link AS il
  ON il.infrastructure_link_id = ilar.infrastructure_link_id
GROUP BY ilar.route_id;
CREATE INDEX tmp_idx_estimatedRouteLengthInMeters_route_id ON route.tmp_estimatedRouteLengthInMeters(routeId);

CREATE MATERIALIZED VIEW route.tmp_lineRoutes AS
...
FROM route.route AS route
LEFT JOIN route.tmp_stopPointsOnRoute ON tmp_stopPointsOnRoute.routeId = route.route_id
LEFT JOIN route.tmp_estimatedRouteLengthInMeters ON tmp_estimatedRouteLengthInMeters.routeId = route.route_id
GROUP BY route.on_line_id;
CREATE INDEX tmp_idx_lineRoutes_line_id ON route.tmp_lineRoutes(lineId);
...
INNER JOIN route.tmp_lineRoutes ON tmp_lineRoutes.lineId = line.line_id;

DROP MATERIALIZED VIEW route.tmp_lineRoutes;
DROP MATERIALIZED VIEW route.tmp_estimatedRouteLengthInMeters;
DROP MATERIALIZED VIEW route.tmp_stopPointsOnRoute;
