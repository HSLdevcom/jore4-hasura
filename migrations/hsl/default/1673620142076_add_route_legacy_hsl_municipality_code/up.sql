CREATE TABLE hsl_route.legacy_hsl_municipality_code (
  legacy_hsl_municipality_code text PRIMARY KEY,
  jore3_code smallint NOT NULL CHECK (0 <= jore3_code AND jore3_code <= 9) -- all possible values are inserted here.
);
...
INSERT INTO hsl_route.legacy_hsl_municipality_code
  (jore3_code, legacy_hsl_municipality_code)
  VALUES
  ...
  ON CONFLICT (legacy_hsl_municipality_code) DO NOTHING;
...
ALTER TABLE route.route
  ADD COLUMN legacy_hsl_municipality_code text NOT NULL REFERENCES hsl_route.legacy_hsl_municipality_code (legacy_hsl_municipality_code) DEFAULT 'helsinki';
