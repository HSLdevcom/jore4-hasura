...
ALTER TABLE timetables.substitute_operating_day_by_line_type
  ...
  CHECK (
    begin_time >= timetables.const_operating_day_start_time()
      AND begin_time < coalesce(end_time, timetables.const_operating_day_end_time())
  );
...
ALTER TABLE timetables.substitute_operating_day_by_line_type
  ...
  CHECK (
    end_time > coalesce(begin_time, timetables.const_operating_day_start_time())
      AND end_time <= timetables.const_operating_day_end_time()
  );
