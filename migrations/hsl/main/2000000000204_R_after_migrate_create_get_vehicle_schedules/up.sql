CREATE OR REPLACE FUNCTION timetables.get_substitute_operating_period_begin_date(substitute_operating_period_uuid uuid)
RETURNS date
LANGUAGE SQL
  STABLE
AS $$
  SELECT MIN(superseded_date)
  FROM timetables.substitute_operating_day_by_line_type
  WHERE substitute_operating_period_id = substitute_operating_period_uuid;
$$
;

COMMENT ON FUNCTION timetables.get_substitute_operating_period_begin_date(substitute_operating_period_uuid uuid)
IS 'Returns the begin date of a substitute operating period that consists of substitute_operating_day_by_line_types
(superseded_date).
';

CREATE OR REPLACE FUNCTION timetables.get_substitute_operating_period_end_date(substitute_operating_period_uuid uuid)
RETURNS date
LANGUAGE SQL
  STABLE
AS $$
...
COMMENT ON FUNCTION timetables.get_substitute_operating_period_end_date(substitute_operating_period_uuid uuid)
IS 'Returns the last date of a substitute operating period that consists of substitute_operating_day_by_line_types
(superseded_date).
';

CREATE OR REPLACE FUNCTION timetables.get_vehicle_schedules_on_date(journey_pattern_uuid uuid, observation_date date)
...
(
  SELECT DISTINCT vj.vehicle_journey_id,
    timetables.get_substitute_operating_period_begin_date(sodblt.substitute_operating_period_id),
    timetables.get_substitute_operating_period_end_date(sodblt.substitute_operating_period_id),
    ...
    ( -- calculate what is the correct day type id for the superseded date
      SELECT day_type_id
      FROM timetables.get_active_day_types_for_date(sodblt.superseded_date) dt
        JOIN  timetables.day_type_active_on_day_of_week dtaodow USING(day_type_id)
  ...
  FROM timetables.substitute_operating_day_by_line_type sodblt
    JOIN timetables.journey_pattern_ref jpr USING(type_of_line)
    JOIN timetables.vehicle_journey vj USING(journey_pattern_ref_id)
    JOIN timetables.block b USING (block_id)
    JOIN timetables.vehicle_service vs USING (vehicle_service_id)
    JOIN timetables.vehicle_schedule_frame vsf USING (vehicle_schedule_frame_id)
    JOIN timetables.day_type_active_on_day_of_week dtaodow USING (day_type_id)
  WHERE jpr.journey_pattern_id = journey_pattern_uuid
    ...
    AND vsf.priority != internal_utils.const_priority_draft()
...
(
  ...
  FROM timetables.vehicle_schedule_frame vsf
    JOIN timetables.vehicle_service vs USING (vehicle_schedule_frame_id)
    JOIN timetables.day_type dt USING(day_type_id)
    JOIN timetables.block b USING(vehicle_service_id)
    JOIN timetables.vehicle_journey vj USING(block_id)
    JOIN timetables.journey_pattern_ref jpr USING (journey_pattern_ref_id)
  WHERE jpr.journey_pattern_id = journey_pattern_uuid
    AND vsf.priority != internal_utils.const_priority_staging()
    AND vsf.priority != internal_utils.const_priority_draft()
...
COMMENT ON FUNCTION timetables.get_vehicle_schedules_on_date(journey_pattern_uuid uuid, observation_date date)
IS 'Returns all valid highest priority vehicle schedules for a journey pattern on an observation date. Uses return_value.vehicle_schedule as the return value
