CREATE OR REPLACE FUNCTION timetables.get_timetables_and_substitute_operating_days(journey_pattern_ids uuid[], start_date date, end_date date)
...
SELECT DISTINCT vsf.vehicle_schedule_frame_id, -- vehicle_schedule_frame_id
  ...
  dt.day_type_id -- day_type_id
    FROM timetables.vehicle_schedule_frame vsf
      ...
      JOIN timetables.journey_patterns_in_vehicle_service jp_in_vs USING(vehicle_service_id)
    WHERE jp_in_vs.journey_pattern_id = ANY(journey_pattern_ids)
    AND vsf.validity_end >= start_date
    AND vsf.validity_start <= end_date
    AND vsf.priority != internal_utils.const_priority_staging() -- ignore staging priorities completely, since they are not shown anywhere
UNION
SELECT DISTINCT
...
FROM timetables.journey_pattern_ref jpr
  JOIN timetables.substitute_operating_day_by_line_type sodblt USING(type_of_line)
...
COMMENT ON FUNCTION timetables.get_timetables_and_substitute_operating_days
IS 'Uses return_value.timetable_version as return value. Returns all the corresponding data from
timetables.vehicle_schedule_frame and timetables.substitute_operating_day_by_line_type that matches the given timerange and journey_pattern,
...
CREATE OR REPLACE FUNCTION timetables.get_timetable_versions_by_journey_pattern_ids(journey_pattern_ids uuid[], start_date date, end_date date, observation_date date)
  RETURNS SETOF return_value.timetable_version
  LANGUAGE SQL
  STABLE
AS $$
...
(
  ...
  FROM timetables.get_timetables_and_substitute_operating_days(journey_pattern_ids, start_date, end_date)
)
SELECT
  ...
  ( -- calculate the in_effect value
    ...
    AND timetable_versions.priority != internal_utils.const_priority_draft() -- do not set in effect value to drafts, staging priority is already omitted in get_timetables_and_substitute_operating_days
    AND
    (
      timetable_versions.priority = internal_utils.const_priority_special()
      OR NOT EXISTS
      (
        SELECT 1
        ...
        WHERE
          ...
          AND overriding_tv.priority != internal_utils.const_priority_draft() -- do not override other priorities with draft priority
...
COMMENT ON FUNCTION timetables.get_timetable_versions_by_journey_pattern_ids
