...
CREATE TABLE IF NOT EXISTS network.line_change_history (
  ...
  route_direction TEXT NULL DEFAULT NULL, --- Used to render the initial block-of-changes header
...
CREATE INDEX IF NOT EXISTS idx_line_and_priority ON network.line_change_history(line_label);
...
CREATE UNIQUE INDEX IF NOT EXISTS history_unique_line_route_combo_per_transaction
  ON network.line_change_history(line_id, route_id, changed);

--- Add comments
COMMENT ON TABLE network.line_change_history IS
  'Collects and represents the historical states of an HSL Lines.';

COMMENT ON COLUMN network.line_change_history.id IS
  'Auto-incremented serial db row ID.';

COMMENT ON COLUMN network.line_change_history.tg_operation IS
  'DB operation type (INSERT,UPDATE,DELETE) that triggered the creation of this history item.';

COMMENT ON COLUMN network.line_change_history.line_label IS
  'DB ID of the HSL Line this historical record represents.';
COMMENT ON COLUMN network.line_change_history.line_label IS
  'Label of the HSL Line this historical record represents.';
COMMENT ON COLUMN network.line_change_history.line_priority IS
  'Priority of the HSL Line this historical record represents.';

COMMENT ON COLUMN network.line_change_history.line_validity_start IS
  'Validity start date of the HSL line as it was at time of the change.';
COMMENT ON COLUMN network.line_change_history.line_validity_end IS
  'Validity end date of the HSL line as it was at time of the change.';

COMMENT ON COLUMN network.line_change_history.route_id IS
  'DB ID of the HSL route that was changed (inserted, updated, deleted).';
COMMENT ON COLUMN network.line_change_history.route_label IS
  'Label of the HSL route that was changed.';
COMMENT ON COLUMN network.line_change_history.route_direction IS
  'Label of the HSL route that was changed.';
COMMENT ON COLUMN network.line_change_history.route_validity_start IS
  'Validity start date of the HSL route as it was at time of the change, if a route change triggered this history item.';
COMMENT ON COLUMN network.line_change_history.route_validity_end IS
  'Validity end date of the HSL route as it was at time of the change, if a route change triggered this history item.';

COMMENT ON COLUMN network.line_change_history.name IS
  'Finnish name of the HSL line or route, if a route change triggered this history item.';
COMMENT ON COLUMN network.line_change_history.version_comment IS
  'Version comment of the line or route, if a route change triggered this history item.';

COMMENT ON COLUMN network.line_change_history.changed IS
  'Time when this history item was recorded / the change occurred';
COMMENT ON COLUMN network.line_change_history.changed_by IS
  'UserID of the user who made the change, empty on automated system operations';

COMMENT ON COLUMN network.line_change_history.data IS
  'JSONB blob containing any and all relevant data from the associated line row with full graphs of any extra data (routes) needed to visualize the changes made. See `network.collect_line_info_for_history` function for implementation details.';


--- Create a function to construct a historical snapshot of the line and
--- of any related relevant related rows.
CREATE OR REPLACE FUNCTION
  network.collect_line_info_for_history(lineId UUID)
  RETURNS jsonb
  LANGUAGE plpgsql
  STABLE AS $$
DECLARE
...
BEGIN
  ...
  SELECT to_jsonb(line.*) INTO line FROM network.line WHERE line_id = lineId;
  ...
  FOR routeId IN SELECT route_id FROM network.route WHERE on_line_id = lineId LOOP
    --- Fetch a list of stops for the route
    SELECT jsonb_agg(to_jsonb(sspijp.*) ORDER BY sspijp.scheduled_stop_point_sequence) INTO stopPointsOnRoute
    FROM network.journey_pattern AS jp
    INNER JOIN network.scheduled_stop_point_in_journey_pattern AS sspijp
    ...
    FROM network.infrastructure_link_along_route AS ilar
    ...
    SELECT to_jsonb(route.*) || jsonb_build_object(
      'stops', COALESCE(stopPointsOnRoute, '[]'::JSONB),
      'estimated_length_in_metres', estimatedRouteLengthInMeters
    ) INTO route
    FROM network.route AS route
...
CREATE OR REPLACE FUNCTION
  network.get_higher_priority_operation(previousOperation TEXT, nextOperation TEXT)
  RETURNS TEXT
  LANGUAGE plpgsql
  IMMUTABLE AS $$
BEGIN
...
END;
$$;
COMMENT ON FUNCTION network.get_higher_priority_operation(previousOperation TEXT, nextOperation TEXT)
...
CREATE OR REPLACE FUNCTION
  network.record_history_for_changed_line(tgOperation TEXT, lineId UUID)
  RETURNS VOID
  LANGUAGE plpgsql
  VOLATILE AS $$
BEGIN
  INSERT INTO network.line_change_history AS lch (
    tg_operation,
    line_id,
    line_label,
    line_priority,
  ...
  SELECT
    ...
    network.collect_line_info_for_history(line.line_id)
  FROM network.line
  WHERE line.line_id = lineId
  ON CONFLICT (line_id, route_id, changed)
  DO UPDATE SET
    tg_operation=network.get_higher_priority_operation(lch.tg_operation, EXCLUDED.tg_operation),
...
CREATE OR REPLACE FUNCTION
  network.record_history_after_line_upserted()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  VOLATILE AS $$
BEGIN
  EXECUTE network.record_history_for_changed_line(TG_OP, NEW.line_id);
  RETURN NULL;
END;
$$;

CREATE OR REPLACE TRIGGER record_history_after_line_upserted
AFTER INSERT OR UPDATE ON network.line FOR EACH ROW
EXECUTE FUNCTION network.record_history_after_line_upserted();


--- Create the trigger for line DELETE events.
CREATE OR REPLACE FUNCTION
  network.record_history_before_line_deleted()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  VOLATILE AS $$
BEGIN
  EXECUTE network.record_history_for_changed_line(TG_OP, OLD.line_id);
  RETURN OLD;
END;
$$;

CREATE OR REPLACE TRIGGER record_history_before_line_deleted
BEFORE DELETE ON network.line FOR EACH ROW
EXECUTE FUNCTION network.record_history_before_line_deleted();


--- Create the trigger for route INSERT & UPDATE events
CREATE OR REPLACE FUNCTION
  network.record_history_after_route_upserted()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  VOLATILE AS $$
DECLARE
  line network.line := NULL;
...
BEGIN
  ...
  SELECT * INTO line FROM network.line WHERE line_id = NEW.on_line_id;
  newLineData = network.collect_line_info_for_history(NEW.on_line_id);
  ...
  IF (OLD.on_line_id != NEW.on_line_id) THEN
    oldLineData := network.collect_line_info_for_history(NEW.on_line_id);
...
AFTER INSERT OR UPDATE ON network.route FOR EACH ROW
EXECUTE FUNCTION network.record_history_after_route_upserted();


--- Create a helper function and the trigger for route DELETE events.
CREATE OR REPLACE FUNCTION
  network.record_history_for_changed_route(tgOperation TEXT, routeId UUID)
  RETURNS VOID
  LANGUAGE plpgsql
  VOLATILE AS $$
BEGIN
  ...
  FROM network.route
  INNER JOIN network.line ON line.line_id = route.on_line_id
...
CREATE OR REPLACE FUNCTION
  network.record_history_before_route_deleted()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  VOLATILE AS $$
BEGIN
  EXECUTE network.record_history_for_changed_route(TG_OP, OLD.route_id);
  RETURN OLD;
END;
$$;

CREATE OR REPLACE TRIGGER record_history_before_route_deleted
BEFORE DELETE ON network.route FOR EACH ROW
EXECUTE FUNCTION network.record_history_before_route_deleted();
...
CREATE OR REPLACE FUNCTION
  network.record_history_after_route_infra_links_changed()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  VOLATILE AS $$
DECLARE
...
BEGIN
  ...
  LOOP
    ...
    EXECUTE network.record_history_for_changed_route('UPDATE', id);
  END LOOP;
...
CREATE OR REPLACE TRIGGER record_history_after_route_infra_links_inserted
  AFTER INSERT ON network.infrastructure_link_along_route
  REFERENCING NEW TABLE AS new_links_along_route
  FOR EACH STATEMENT
EXECUTE FUNCTION network.record_history_after_route_infra_links_changed();

CREATE OR REPLACE TRIGGER record_history_after_route_infra_links_updated
AFTER UPDATE ON network.infrastructure_link_along_route
...
AFTER DELETE ON network.infrastructure_link_along_route
...
--- Record snapshot for each unique route changed (there should always be only one).
CREATE OR REPLACE FUNCTION
  network.record_history_after_route_journey_pattern_stop_points_changed()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  VOLATILE AS $$
DECLARE
...
BEGIN
  ...
  LOOP
    --- Actual operation might not be UPDATE, but from the Route's perspective, changes to its
    --- children, constitute an "UPDATE" of its "aggregate state".
    EXECUTE network.record_history_for_changed_route('UPDATE', idRecord.on_route_id);
  END LOOP;
...
CREATE OR REPLACE TRIGGER record_history_after_route_journey_pattern_stop_points_inserted
  AFTER INSERT ON network.scheduled_stop_point_in_journey_pattern
  REFERENCING NEW TABLE AS new_sspijp
  FOR EACH STATEMENT
EXECUTE FUNCTION network.record_history_after_route_journey_pattern_stop_points_changed();

CREATE OR REPLACE TRIGGER record_history_after_route_journey_pattern_stop_points_updated
  AFTER UPDATE ON network.scheduled_stop_point_in_journey_pattern
...
CREATE OR REPLACE TRIGGER record_history_after_route_journey_pattern_stop_points_deleted
  AFTER DELETE ON network.scheduled_stop_point_in_journey_pattern
...
CREATE MATERIALIZED VIEW network.tmp_stopPointsOnRoute AS
SELECT jp.on_route_id AS routeId,
       jsonb_agg(to_jsonb(sspijp.*) ORDER BY sspijp.scheduled_stop_point_sequence) AS stops
FROM network.journey_pattern AS jp
INNER JOIN network.scheduled_stop_point_in_journey_pattern AS sspijp
  ON sspijp.journey_pattern_id = jp.journey_pattern_id
GROUP BY jp.on_route_id;
CREATE INDEX tmp_idx_stopPointsOnRoute_route_id ON network.tmp_stopPointsOnRoute(routeId);

CREATE MATERIALIZED VIEW network.tmp_estimatedRouteLengthInMeters AS
SELECT ilar.route_id AS routeId, sum(il.estimated_length_in_metres) AS length
FROM network.infrastructure_link_along_route AS ilar
INNER JOIN infrastructure_network.infrastructure_link AS il
  ON il.infrastructure_link_id = ilar.infrastructure_link_id
GROUP BY ilar.route_id;
CREATE INDEX tmp_idx_estimatedRouteLengthInMeters_route_id ON network.tmp_estimatedRouteLengthInMeters(routeId);

CREATE MATERIALIZED VIEW network.tmp_lineRoutes AS
SELECT route.on_line_id AS lineId, jsonb_agg(to_jsonb(route.*) || jsonb_build_object(
  'stops', COALESCE(tmp_stopPointsOnRoute.stops, '[]'::JSONB),
  'estimated_length_in_metres', tmp_estimatedRouteLengthInMeters.length
)) AS routes
FROM network.route AS route
LEFT JOIN network.tmp_stopPointsOnRoute ON tmp_stopPointsOnRoute.routeId = route.route_id
LEFT JOIN network.tmp_estimatedRouteLengthInMeters ON tmp_estimatedRouteLengthInMeters.routeId = route.route_id
GROUP BY route.on_line_id;
CREATE INDEX tmp_idx_lineRoutes_line_id ON network.tmp_lineRoutes(lineId);
...
SELECT
  ...
  to_jsonb(line.*) || jsonb_build_object(
    'routes', COALESCE(tmp_lineRoutes.routes, '[]'::JSONB)
  )
FROM network.line
INNER JOIN network.route ON route.on_line_id = line.line_id
INNER JOIN network.tmp_lineRoutes ON tmp_lineRoutes.lineId = line.line_id;

DROP MATERIALIZED VIEW network.tmp_lineRoutes;
DROP MATERIALIZED VIEW network.tmp_estimatedRouteLengthInMeters;
DROP MATERIALIZED VIEW network.tmp_stopPointsOnRoute;
