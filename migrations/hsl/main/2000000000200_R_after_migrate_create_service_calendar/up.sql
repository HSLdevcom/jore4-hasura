CREATE OR REPLACE FUNCTION timetables.get_active_day_types_for_date(observation_date date)
  RETURNS SETOF timetables.day_type
  LANGUAGE sql STABLE
AS $$
  SELECT dt.*
    ...
    LEFT JOIN timetables.day_type_active_on_day_of_week dtaodow ON dtaodow.day_type_id = dt.day_type_id
...
$$;
COMMENT ON FUNCTION timetables.get_active_day_types_for_date IS 'Finds all the day types that are active on the given observation date.
