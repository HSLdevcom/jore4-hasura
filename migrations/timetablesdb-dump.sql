...
ALTER TABLE ONLY service_calendar.substitute_operating_day
    ADD CONSTRAINT substitute_operating_day_no_timespan_overlap EXCLUDE USING gist (type_of_line WITH =, superseded_date WITH =, tsrange(begin_datetime, end_datetime, '[]'::text) WITH &&);
...
CREATE UNIQUE INDEX substitute_operating_day_uniqueness ON service_calendar.substitute_operating_day USING btree (type_of_line, superseded_date, no_traffic);
...
CREATE TABLE service_calendar.substitute_operating_day (
    ...
    begin_time interval,
    end_time interval,
    begin_datetime timestamp without time zone GENERATED ALWAYS AS ((begin_time + superseded_date)) STORED,
    end_datetime timestamp without time zone GENERATED ALWAYS AS ((end_time + superseded_date)) STORED,
