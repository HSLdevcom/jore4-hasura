...
CREATE FUNCTION internal_route.check_route_line_validity_period()
  RETURNS trigger
  LANGUAGE plpgsql AS
$$
DECLARE
...
CREATE CONSTRAINT TRIGGER check_route_line_validity_periods_trigger
  AFTER INSERT OR UPDATE ON internal_route.route
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW EXECUTE PROCEDURE internal_route.check_route_line_validity_period();


CREATE FUNCTION route.check_line_routes_validity_periods()
  RETURNS trigger
  LANGUAGE plpgsql AS
$$
BEGIN
...
CREATE CONSTRAINT TRIGGER check_line_routes_validity_periods_trigger
  AFTER UPDATE ON route.line
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW EXECUTE PROCEDURE route.check_line_routes_validity_periods();
